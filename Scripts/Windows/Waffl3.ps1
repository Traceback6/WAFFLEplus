...
if ($option -eq 1) {
    ...
    Write-Warning "catching them special homework files"
    ...
    # (the following is commented out to avoid confusion when in compettition)
...
if ($option -eq 2) {
    ...
    $proclist = (get-nettcpconnection | Where-Object { $_.State -eq 'Listen' }).OwningProcess
    ...
    foreach ($proc in $proclist) {
        ...
        $port = (Get-NetTCPConnection | Where-Object { $_.OwningProcess -eq $proc }).LocalPort
    ...
    $tcpcon | Sort-Object Name | Format-Table -AutoSize
}
...
if ($option -eq 3) {
    ...
    $string = Read-Host 
    Get-ChildItem -r $absolutepath -file | ForEach-Object { Select-String -path $_ -pattern $string } 
} # .-- .- ..-. ..-. .-.. ...-- 
